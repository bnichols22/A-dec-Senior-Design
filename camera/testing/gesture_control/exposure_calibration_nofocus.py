...
# Focus has been removed from this list.
param_ranges = {
...
# The actions we want to map gestures to (Focus removed)
actions_to_map = ["exposure", "brightness", "contrast", "white_balance", "SAVE_PROFILE"]
...
def apply_camera_settings(cap, settings):
    ...
    # WHITE BALANCE
    # Note: Setting auto_wb to 0 is often required before setting temperature manually
...
def get_fingers_status(hand_landmarks, handedness_label):
    ...
    if handedness_label == "Left": 
        # For Left hand, check if thumb is to the right of the IP joint (relative to image)
