...
_baseline_valid = False  # NEW: do not send until we have a valid baseline


def get_board_angles(retries=30, delay_s=0.05):
    """
    Read current board angles (yaw, pitch, roll) using bgc_get_angles.
    Returns (yaw, pitch, roll) floats on success, or None on failure.

    The retry loop prevents the "first movement bug" where baseline defaults to 0
    because the board isn't ready yet.
    """
    global _bgc_lib, _bgc_initialized

    if TEST == 1:
        # In TEST mode, pretend the board is at 0,0,0
        return (0.0, 0.0, 0.0)

    if _bgc_lib is None or not _bgc_initialized:
        return None

    if getattr(_bgc_lib, "bgc_get_angles", None) is None:
        return None
...
def init_sbgc():
    ...
    # NEW: Robust baseline capture (retry), and do NOT mark valid if it fails.
    ...
    if angles is not None:
        ...
        _baseline_valid = False
        print("# WARN: Could not read baseline angles from board at startup.")
        print("#       Will retry lazily on first send to avoid a 0,0,0 jump.")
...
def ensure_baseline():
    """
    NEW: Make sure baseline is valid before any send.
    If not valid, try to capture it now. Returns True if valid after this call.
    """
    global _baseline_yaw_deg, _baseline_pitch_deg, _baseline_roll_deg, _baseline_valid
    ...
    print("# Baseline angles captured lazily (first send):")
    print(f"#   yaw={_baseline_yaw_deg:.2f}, pitch={_baseline_pitch_deg:.2f}, roll={_baseline_roll_deg:.2f}")
    return True
...
def send_or_log_angles(d_yaw_deg, d_pitch_deg, d_roll_deg,
    ...
    """
    if TEST != 1:
        # NEW: prevent first movement bug (do not send with baseline=0 unless it's truly correct)
        if not ensure_baseline():
            # Skip sending until baseline is available.
            print("# WARN: baseline not ready; skipping send to avoid 0,0,0 jump.")
            return False
