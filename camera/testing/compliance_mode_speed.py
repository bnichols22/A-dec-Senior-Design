...
#   Adds a simple "compliance mode":
#     - Press 'c' to toggle compliance
#     - Compliance ON: motors OFF so user can reposition by hand
#     - Compliance OFF: motors ON and hold (0 speed), tracking resumes
# ==============================================================
...
# SPEED controller tuning (more precise: a bit lower + more smoothing)
...
def init_sbgc():
    ...
    if not hasattr(_bgc_lib, "bgc_control_speeds"):
        print("# ERROR: libsimplebgc.so missing bgc_control_speeds(). Rebuild .so.")
        _bgc_initialized = False
        return
    ...
    if not hasattr(_bgc_lib, "bgc_set_motors"):
        print("# ERROR: libsimplebgc.so missing bgc_set_motors(). Rebuild .so.")
        _bgc_initialized = False
        return
...
def set_compliance(enable: bool):
    """
    Soft compliance:
      - enable=True  -> motors OFF (user can reposition)
      - enable=False -> motors ON + send 0 speeds to hold current orientation
    """
    if TEST == 1:
        print(f"# TEST: compliance={'ON' if enable else 'OFF'}")
        return True

    if _bgc_lib is None or not _bgc_initialized:
        print("# ERROR: SBGC shim not initialized (compliance toggle).")
        return False

    if enable:
        # Stop motion then release
        send_or_log_speeds(0.0, 0.0, 0.0)
        rc = _bgc_lib.bgc_set_motors(0)
        if rc != 0:
            print(f"# ERROR: bgc_set_motors(0) rc={rc}")
            return False
        return True

    # Disable compliance: re-enable motors and "lock in"
    rc = _bgc_lib.bgc_set_motors(1)
    if rc != 0:
        print(f"# ERROR: bgc_set_motors(1) rc={rc}")
        return False

    # Send a couple of zero-speed commands so it holds wherever it is
    send_or_log_speeds(0.0, 0.0, 0.0)
    time.sleep(0.05)
    send_or_log_speeds(0.0, 0.0, 0.0)
    return True
...
def main():
    ...
    print("# Keys: ESC=quit, c=toggle compliance")
    ...
    while True:
        ...
        # If compliance is ON, don't do tracking control at all.
        if compliance:
            # Just display + keep sending nothing
            if (now - last_send_time) >= COMMAND_PERIOD:
                # don't spam, but keep "quiet"
                last_send_time = now

            if DRAW:
                cv2.putText(frame, "COMPLIANCE MODE (motors off) - press 'c' to lock",
                            (10, 28), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 255), 2, cv2.LINE_AA)
        ...
        if DRAW:
            ...
            cv2.putText(frame, "Press 'c' for compliance", (10, 72),
    ...
    # if we quit while in compliance, re-enable motors for safety
    try:
        if compliance:
            set_compliance(False)
    except Exception:
