...
#   (2) Compliance + lock modes controlled by a physical pushbutton on GPIO 6:
#       - Press once  -> MOTORS OFF (compliance; user can reposition by hand)
#       - Press twice -> MOTORS ON, HOLD (0 speed) but TRACKING DISABLED
#       - Press third -> TRACKING ENABLED again (normal operation)
#       Then repeat.
#
# Motor lib usage:
...
#
# Button wiring (Raspberry Pi):
#   GPIO6 (BCM 6, physical pin 31) ----[button]---- GND
...
def main():
    ...
    # ==============================================================
    # NEW: Pushbutton control for compliance/lock cycle (GPIO 6)
    # Wiring: GPIO6 (BCM 6, physical pin 31) -> button -> GND
    # Using internal pull-up, so pressed = active (LOW).
    # ==============================================================
    mode_button = Button(6, pull_up=True, bounce_time=0.15)
    ...
    # ==============================================================
    # Compliance / lock / tracking state (3-press cycle)
    # ==============================================================
    TRACKING_ENABLED = 0
    ...
    # ==============================================================
    # NEW: Button press handler (thread-safe)
    # ==============================================================
    mode_lock = threading.Lock()

    def handle_button_press():
        nonlocal mode, prev_smoothed, prev_time, consecutive_lost_frames, state
    ...
    mode_button.when_pressed = handle_button_press
    ...
    while True:
        ...
        # ==============================================================
        # Compliance / hold modes (no face tracking)
        # ==============================================================
        with mode_lock:
            local_mode = mode

        if local_mode != TRACKING_ENABLED:
            ...
            if (current_time - last_send_time) >= COMMAND_PERIOD:
                if local_mode == HOLD_MOTORS_ON_NO_TRACK:
                    send_speeds(0.0, 0.0, 0.0)
                last_send_time = current_time

            if DRAW_FRAME_RT:
                if local_mode == COMPLIANCE_MOTORS_OFF:
                    msg = "COMPLIANCE (motors OFF) - press button to lock motors ON"
                    color = (0, 0, 255)
                else:
                    msg = "LOCKED HOLD (motors ON) - press button to reenable tracking"
                    color = (0, 200, 255)

                cv2.putText(frame, msg, (10, 28),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.6, color, 2, cv2.LINE_AA)
                cv2.putText(frame, "ESC to exit", (10, 56),
        ...
        if centroid is None:
            ...
            if DRAW_FRAME_RT:
                ...
                cv2.putText(frame, "Press button for compliance/lock cycle", (10, 56),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255,255,255), 2, cv2.LINE_AA)
        ...
        # Compute the speed commands to send
        ...
        if DRAW_FRAME_RT:
            ...
            cv2.putText(frame, "Button on GPIO6 toggles compliance/lock cycle", (10, 96),
