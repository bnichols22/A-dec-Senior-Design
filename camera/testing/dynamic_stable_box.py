...
#   Eliminates the initial snap-to-zero bug because we never send
#   an absolute angle setpoint on startup.
#
# ADDITION:
#   Stable box size changes in *discrete ranges* based on face distance
#   proxy (eye distance in pixels). Closer face => bigger stable box.
#
# Transport:
...
def estimate_eye_dist_px(face_landmarks, w, h):
    """Return eye distance (px) using FaceMesh landmarks. Returns None if unavailable."""
    try:
        lx = face_landmarks.landmark[EYE_L_IDX].x * w
        ly = face_landmarks.landmark[EYE_L_IDX].y * h
        rx = face_landmarks.landmark[EYE_R_IDX].x * w
        ry = face_landmarks.landmark[EYE_R_IDX].y * h
...
def main():
    ...
    # NEW: stable-box range state
    stable_range = "MID"
    pending_range = None
    pending_count = 0
    stable_scalar = STABLE_SCALAR_DEFAULT

    print("# T yaw_dps pitch_dps sent state r eye_px box_scalar range")
    ...
    while True:
        ...
        if res.multi_face_landmarks:
            ...
            eye_dist_px = estimate_eye_dist_px(face_lm, w, h)
        ...
        print(f"{T:.3f} {yaw_dps:+.2f} {pitch_dps:+.2f} {sent} {state} r={radial_norm:.3f} eye={eye_str} box={stable_scalar:.3f} {stable_range}")
